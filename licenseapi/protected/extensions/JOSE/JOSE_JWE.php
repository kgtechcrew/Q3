...
class JOSE_JWE extends JOSE_JWT {
    ...
    public function rsa($public_or_private_key, $padding_mode) {
    ...
    public function generateContentEncryptionKey($public_key_or_secret) {
    ...
    public function encryptContentEncryptionKey($public_key_or_secret) {
    ...
    public function decryptContentEncryptionKey($private_key_or_secret) {
    ...
    public function deriveEncryptionAndMacKeys() {
        switch ($this->header['enc']) {
            case 'A128GCM':
            case 'A256GCM':
    ...
    public function deriveEncryptionAndMacKeysCBC($sha_size) {
    ...
    public function encryptCipherText() {
        $cipher = $this->cipher();
    ...
    public function generateAuthenticationTag() {
        $this->authentication_tag = $this->calculateAuthenticationTag();
    }

    public function calculateAuthenticationTag($use_raw = false) {
        switch ($this->header['enc']) {
            case 'A128GCM':
            case 'A256GCM':
                throw new JOSE_Exception_UnexpectedAlgorithm('Algorithm not supported');
    ...
    public function calculateAuthenticationTagCBC($sha_size) {
        if (!$this->auth_data) {
    ...
    public function checkAuthenticationTag() {
